...
: "${CTRTOOL:=ctrtool}"
export CTRTOOL
unshare --fork -r -m -p --propagation=slave --mount-proc "$CTRTOOL" launcher -n --alloc-tty --script-is-shell -U --escape -m --uid-map=0.0.1 --gid-map=0.0.1 --no-clear-groups --script='./test-exampleWithFilters.sh self/fd/"$2" "" X &' sh -eu -c '
ip link set lo up
